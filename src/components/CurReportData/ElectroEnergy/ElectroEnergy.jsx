...
export const ElectroEnergy = ({
  ...
  const { data: reportData } = useGetCurDataQuery({
    endYear: +endYear,
    endPeriod: +endPeriod,
    orgId: +organization,
  });
          ...
          <thead>
            <tr>
              ...
              <th colSpan={2} rowSpan={2}>
                факт {endPeriodText} {startYear} года
              </th>
              <th colSpan={12}>
                {endPeriodText} {endYear} года
              </th>
            </tr>
            <tr>
              ...
              <th colSpan={4}>
                отклонение {endYear} года от {startYear} года
              </th>
            </tr>
            <tr>
              <th>кВт*час</th>
            ...
            {reportData &&
              reportData.dataView.map((item, index) => (
                <tr key={`report-data-${index}`}>
                  ...
                  <td className={style["bold"]}>
                    {item.electro.endData.planValue}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.endData.planSum}
                  </td>
                  <td className={style["bold"]}>
                  ...
                  <td className={style["bold"]}>
                    {item.electro.deviation.sum}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.deviation.percentSum}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.deviationYear.value}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.deviationYear.percent}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.deviationYear.sum}
                  </td>
                  <td className={style["bold"]}>
                    {item.electro.deviationYear.percentSum}
                  </td>
                </tr>
              ))}
          </tbody>
          <tfoot>
            {reportData && (
              <tr>
                ...
                <td>{reportData.totalData.electro.endData.planValue}</td>
                <td>{reportData.totalData.electro.endData.planSum}</td>
                ...
                <td>
                  {reportData.totalData.electro.endData.value != 0
                    ? reportData.totalData.electro.endData.planValue != 0
                      ? Math.round(
                          (100 * reportData.totalData.electro.endData.value) /
                            reportData.totalData.electro.endData.planValue -
                            100
                        )
                      : 0
                    : 0}
                </td>
                <td>{reportData.totalData.electro.deviation.sum}</td>
                <td>
                  {reportData.totalData.electro.endData.sum != 0
                    ? reportData.totalData.electro.endData.planSum != 0
                      ? Math.round(
                          (100 * reportData.totalData.electro.endData.sum) /
                            reportData.totalData.electro.endData.planSum -
                            100
                        )
                      : 0
                    : 0}
                ...
                <td>
                  {reportData.totalData.electro.deviationYear &&
                    reportData.totalData.electro.deviationYear.value}
                </td>
                <td>
                  {reportData.totalData.electro.endData.value != 0
                ...
                <td>
                  {reportData.totalData.electro.deviationYear &&
                    reportData.totalData.electro.deviationYear.sum}
                </td>
                <td>
                  {reportData.totalData.electro.endData.sum != 0
                    ? reportData.totalData.electro.startData.sum != 0
                      ? Math.round(
                          (100 * reportData.totalData.electro.endData.sum) /
                            reportData.totalData.electro.startData.sum -
                            100
                        )
                      : 0
                    : 0}
