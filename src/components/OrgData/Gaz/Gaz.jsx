...
export const Gaz = ({ year, period, organizationId }) => {
  ...
  const handleExportClick = () => {
    ...
    downloadWorkbook(workbook, 'По потреблению газа_' + dayjs().format('YYYY-MM-DD HH:mm:ss') + '.xlsx');
  };
    ...
    <div className='row' style={{ paddingLeft: '5vh', paddingTop: '5vh' }}>
      ...
      <div className='row'>
        <table className='table table-hover' id="my-table">
          ...
          <tbody>
            {reportData && reportData.gaz.map((item, index) => (
              <_Data key={`data-1234-${index}`}
                item={item}
              />
            ))}
          </tbody>
          <tfoot>
            {reportData &&
              <tr>
                <td>{reportData.gazTotalData.objectName}</td>
                <td>{reportData.gazTotalData.planValue}</td>
                <td>{reportData.gazTotalData.planSum}</td>
                <td>{reportData.gazTotalData.value}</td>
                <td>{reportData.gazTotalData.sum}</td>
                <td></td>
              </tr>
            }
          </tfoot>
